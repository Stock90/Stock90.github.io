document.addEventListener("DOMContentLoaded", () => {
  ...
  /* =============================
     META TAGS DINÁMICOS POR MARCA
  ============================== */
 
  const marcaInfo = {
    akg:       { nombre: "AKG",                   tipo: "micrófonos y audífonos profesionales" },
    crown:     { nombre: "Crown",                 tipo: "amplificadores profesionales" },
    dbx:       { nombre: "DBX",                   tipo: "procesadores de audio" },
    elation:   { nombre: "Elation",               tipo: "iluminación profesional" },
    jbl:       { nombre: "JBL",                   tipo: "altavoces y sistemas de sonido" },
    kym:       { nombre: "König & Meyer",         tipo: "soportes y accesorios para audio" },
    martin:    { nombre: "Martin",                tipo: "iluminación profesional" },
    obsidian:  { nombre: "Obsidian",              tipo: "consolas y controladores DMX" },
    powersoft: { nombre: "Powersoft",             tipo: "amplificadores profesionales" },
    prolyte:   { nombre: "Prolyte",               tipo: "estructuras y truss para eventos" },
    se:        { nombre: "SE Electronics",        tipo: "micrófonos de estudio" },
    soundcraft: { nombre: "Soundcraft",           tipo: "consolas de mezcla" },
  };
 
  const info = marcaInfo[marca] || { nombre: hoja, tipo: "equipos profesionales" };
 
  const titulo   = `${info.nombre} | Catálogo de Audio Profesional`;
  const descripcion = `Catálogo de ${info.tipo} ${info.nombre}. Precios actualizados para Colombia. Contáctanos por WhatsApp.`;
  const urlCanonica = `https://stock90.github.io/catalogo.html?marca=${marca}`;
  const imagenOG = `https://stock90.github.io/assets/icons/${marca}-menu.webp`;
  ...
  setMeta('meta[property="og:url"]',         "content", urlCanonica);
  setMeta('meta[property="og:image"]',       "content", imagenOG);
  setMeta('link[rel="canonical"]',           "href",    urlCanonica);
